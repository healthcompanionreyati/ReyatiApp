...
test("all thirty APIs require an active account, return no-store data, and expose GET only", async () => {
...
test("thirty pages share one bilingual responsive operations workspace", async () => {
  ...
  assert.match(workspace,/ضمان مستندات الإنتاج/); assert.match(workspace,/Production document assurance/); assert.match(workspace,/Live operations/); assert.match(workspace,/Continuous assurance/); assert.match(workspace,/Control assurance/);
