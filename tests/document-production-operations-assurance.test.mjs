...
const routes = [
  ...
  "document-availability-assurance","document-processing-reliability-assurance","document-queue-reliability-assurance","document-service-level-assurance","document-capacity-planning-assurance",
  "document-maintenance-governance-assurance","document-change-risk-assurance","document-operational-readiness-assurance","document-service-reporting-assurance","document-service-management-scorecard",
];

test("sixty production operations modules derive current durable release evidence", async () => {
  const service = await read("lib/document-production-operations.ts");
  assert.match(service, /getDocumentReleaseWorkspace/); assert.match(service, /medical-document-production-operations-v6/);
  for (const stage of ["runtime_controls","storage_watch","scanner_watch","queue_watch","retention_watch","deletion_watch","legal_hold_watch","incident_watch","evidence_renewal","operations_handoff","service_health","sla_watch","capacity_watch","recovery_readiness","vendor_assurance","access_certification","audit_reconciliation","change_calendar","privacy_obligations","executive_assurance","cleanup_assurance","scan_dispatch_assurance","scan_polling_assurance","scan_recovery_assurance","quarantine_assurance","retention_control_assurance","deletion_control_assurance","legal_hold_enforcement","maintenance_readiness","safety_rehearsal_assurance","continuity_assurance","recovery_runbook_assurance","storage_resilience_assurance","scanner_resilience_assurance","lifecycle_resilience_assurance","incident_response_assurance","evidence_continuity_assurance","ownership_continuity_assurance","dependency_resilience_assurance","resilience_scorecard","policy_alignment_assurance","control_ownership_assurance","release_governance_assurance","exception_governance_assurance","risk_signal_assurance","audit_evidence_assurance","separation_of_duties_assurance","review_cadence_assurance","governance_reporting_assurance","governance_scorecard","availability_assurance","processing_reliability_assurance","queue_reliability_assurance","service_level_assurance","capacity_planning_assurance","maintenance_governance_assurance","change_risk_assurance","operational_readiness_assurance","service_reporting_assurance","service_management_scorecard"]) assert.match(service,new RegExp(`${stage}:`));
});
...
test("all sixty APIs require an active account, return no-store data, and expose GET only", async () => {
...
test("sixty pages share one bilingual responsive operations workspace", async () => {
  ...
  assert.match(workspace,/ضمان مستندات الإنتاج/); assert.match(workspace,/Production document assurance/); assert.match(workspace,/Live operations/); assert.match(workspace,/Continuous assurance/); assert.match(workspace,/Control assurance/); assert.match(workspace,/Resilience assurance/); assert.match(workspace,/Governance assurance/); assert.match(workspace,/Service management/);
