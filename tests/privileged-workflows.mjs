...
// Patient experience: appointment-owned structured feedback, low-volume privacy, withdrawal, and synthetic governance.
const experienceBefore = await request("/api/experience", { identity: identities.patient }); const preVisit = experienceBefore.data.opportunities.find(item => item.appointmentId === booking.appointment.id && item.surveyType === "pre_visit"); assert.ok(preVisit); assert.equal(experienceBefore.data.runtime.publicRatings, false);
const experienceSubmitted = await request("/api/experience", { identity: identities.patient, method: "POST", body: { action: "submit", appointmentId: booking.appointment.id, surveyType: "pre_visit", overallRating: 5, accessRating: 4, communicationRating: 5, respectRating: 5, clarityRating: 4, tags: ["easy_booking", "clear_preparation"], locale: "en", consentAcknowledged: true, consentVersion: experienceBefore.data.consentVersion } }); assert.equal(experienceSubmitted.data.status, "submitted"); assert.equal(experienceSubmitted.data.publicRating, false); assert.equal(experienceSubmitted.data.automatedProviderAction, false);
await request("/api/experience", { identity: identities.patient, method: "POST", status: 409, body: { action: "submit", appointmentId: booking.appointment.id, surveyType: "pre_visit", overallRating: 5, accessRating: 5, communicationRating: 5, respectRating: 5, clarityRating: 5, tags: [], locale: "en", consentAcknowledged: true, consentVersion: experienceBefore.data.consentVersion } }); const providerExperience = await request("/api/provider/experience", { identity: identities.provider }); assert.equal(providerExperience.data.preVisit.thresholdMet, false); assert.equal(providerExperience.data.preVisit.responseCount, null);
await request("/api/provider/experience", { identity: identities.patient, status: 403 }); await request("/api/admin/experience", { identity: identities.patient, status: 403 }); const experienceRehearsal = await request("/api/admin/experience", { identity: identities.admin, method: "POST", body: { action: "run_rehearsal" } }); assert.equal(experienceRehearsal.data.result, "pass"); assert.equal(experienceRehearsal.data.scenarioCount, 10); assert.equal(experienceRehearsal.data.responsesCreated, 0); assert.equal(experienceRehearsal.data.providerActionsCreated, 0); assert.equal(experienceRehearsal.data.externalExports, 0);
const experienceWithdrawn = await request("/api/experience", { identity: identities.patient, method: "POST", body: { action: "withdraw", surveyId: experienceSubmitted.data.id, version: experienceSubmitted.data.version } }); assert.equal(experienceWithdrawn.data.status, "withdrawn"); assert.equal(experienceWithdrawn.data.excludedFromAggregates, true);

// Appointment waitlist: patient request, provider-controlled offer, explicit acceptance, and side-effect-free governance.
const waitEarliest = scheduledStart.toISOString().slice(0, 10), waitLatest = new Date(scheduledStart.valueOf() + 21 * 86400000).toISOString().slice(0, 10); const joinedWaitlist = await request("/api/waitlist", { identity: identities.patient, method: "POST", body: { action: "join", serviceLocationId: service.data.id, earliestDate: waitEarliest, latestDate: waitLatest, timePreference: "morning" } }); assert.equal(joinedWaitlist.data.status, "waiting"); assert.equal(joinedWaitlist.data.automaticBooking, false);
let providerWaitlist = await request("/api/provider/waitlist", { identity: identities.provider }); const providerRequest = providerWaitlist.data.requests.find(item => item.id === joinedWaitlist.data.id); assert.ok(providerRequest); const offeredStart = new Date(scheduledStart.valueOf() + 7 * 86400000); const waitlistOffer = await request("/api/provider/waitlist", { identity: identities.provider, method: "POST", body: { action: "offer", requestId: providerRequest.id, version: providerRequest.version, scheduledStart: offeredStart.toISOString() } }); assert.equal(waitlistOffer.data.status, "offered"); assert.equal(waitlistOffer.data.automaticBooking, false);
const patientWaitlist = await request("/api/waitlist", { identity: identities.patient }); const patientRequest = patientWaitlist.data.requests.find(item => item.id === joinedWaitlist.data.id); const activeOffer = patientRequest.offers.find(item => item.id === waitlistOffer.data.id); assert.ok(activeOffer); const acceptedWaitlist = await request("/api/waitlist", { identity: identities.patient, method: "POST", body: { action: "accept_offer", requestId: patientRequest.id, version: patientRequest.version, offerId: activeOffer.id } }); assert.equal(acceptedWaitlist.data.status, "accepted"); assert.equal(acceptedWaitlist.data.automaticBooking, false); assert.ok(acceptedWaitlist.data.appointmentId);
await request("/api/provider/waitlist", { identity: identities.patient, status: 403 }); await request("/api/admin/waitlist", { identity: identities.patient, status: 403 }); const waitlistRehearsal = await request("/api/admin/waitlist", { identity: identities.admin, method: "POST", body: { action: "run_rehearsal" } }); assert.equal(waitlistRehearsal.data.result, "pass"); assert.equal(waitlistRehearsal.data.scenarioCount, 10); assert.equal(waitlistRehearsal.data.requestsCreated, 0); assert.equal(waitlistRehearsal.data.offersCreated, 0); assert.equal(waitlistRehearsal.data.appointmentsCreated, 0); assert.equal(waitlistRehearsal.data.externalMessagesSent, 0);
