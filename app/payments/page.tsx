...
export default function Payments() {
  ...
  return <main className={`payments-shell payments-live-shell ${ar ? "arabic" : ""}`} dir={ar ? "rtl" : "ltr"} id="main-content">
    ...
    <section className="payments-hero"><div><p>{ar ? "حالة مالية مملوكة للحساب" : "ACCOUNT-OWNED FINANCIAL STATUS"}</p><h1>{ar ? "المدفوعات والفواتير" : "Payments & billing"}</h1><span>{ar ? "اطلع على حالة الدفع المسجلة لكل موعد دون الخلط بين تغييرات الجدول وعمليات الاسترداد." : "See the recorded payment state tied to each of your appointments—without confusing schedule changes with refunds."}</span></div><a href="/support">{ar ? "دعم المدفوعات" : "Payment support"}</a></section>
