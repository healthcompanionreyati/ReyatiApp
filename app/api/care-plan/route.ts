...
import { acknowledgeCarePlan, CarePlanConflictError, CarePlanValidationError, getPatientCarePlans, recordCarePlanProgress, requestCarePlanReview } from "@/lib/care-plans";
...
export async function GET() { return handle((userId) => getPatientCarePlans(userId)); }
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  return handle(async (userId) => {
    if (!body) throw new CarePlanValidationError("A JSON object is required");
    if (body.action === "acknowledge") return acknowledgeCarePlan(userId, body);
    if (body.action === "record_progress") return recordCarePlanProgress(userId, body);
    if (body.action === "request_review") return requestCarePlanReview(userId, body);
...
async function handle(operation: (userId: string) => Promise<unknown>, rateScope?: string) {
  try {
    ...
    reportOperationalError("patient.care_plan.failed", error);
