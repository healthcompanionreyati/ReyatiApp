...
import { getSavedCareGovernance, runSavedCareRehearsal, SavedCareValidationError } from "@/lib/saved-care";
...
export async function GET() { return handle((userId) => getSavedCareGovernance(userId)); }
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  return handle(async (userId) => {
    if (!body || body.action !== "run_rehearsal") throw new SavedCareValidationError("action is invalid");
    return runSavedCareRehearsal(userId);
  }, "admin.saved-care");
}
async function handle(operation: (userId: string) => Promise<unknown>, scope?: string) {
  try {
    ...
    reportOperationalError("admin.saved_care.failed", error);
