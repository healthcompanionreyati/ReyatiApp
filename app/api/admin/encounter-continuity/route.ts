...
import { EncounterContinuityValidationError, getEncounterContinuityGovernance, runEncounterContinuityRehearsal } from "@/lib/encounter-continuity";
...
export async function GET() { return handle(userId => getEncounterContinuityGovernance(userId)); }
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  return handle(async userId => {
    if (!body || body.action !== "run_rehearsal") throw new EncounterContinuityValidationError("action is invalid");
    return runEncounterContinuityRehearsal(userId);
  }, "admin.encounter-continuity");
}
async function handle(operation: (userId: string) => Promise<unknown>, rateScope?: string) {
  try {
    ...
    reportOperationalError("admin.encounter_continuity.failed", error);
