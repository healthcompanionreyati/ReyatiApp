...
import { administerPrivacyRightsRequest, getPrivacyRightsAdministration, PrivacyRightsConflictError, PrivacyRightsValidationError, runPrivacyRightsRehearsal } from "@/lib/privacy-rights";
...
export async function GET() { return handle((userId) => getPrivacyRightsAdministration(userId)); }
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  return handle(async (userId) => {
    if (!body) throw new PrivacyRightsValidationError("A JSON object is required");
    if (body.action === "run_rehearsal") return runPrivacyRightsRehearsal(userId);
    return administerPrivacyRightsRequest(userId, body);
...
async function handle(operation: (userId: string) => Promise<unknown>, rateScope?: string) {
  try {
    ...
    reportOperationalError("admin.privacy_rights.failed", error);
