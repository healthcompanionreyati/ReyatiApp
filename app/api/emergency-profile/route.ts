...
import { EmergencyProfileConflictError, EmergencyProfileValidationError, getEmergencyProfile, saveEmergencyProfile } from "@/lib/emergency-profile";
...
export async function GET() { return handle((userId) => getEmergencyProfile(userId)); }
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  return handle(async (userId) => {
    if (!body || body.action !== "save_profile") throw new EmergencyProfileValidationError("action is invalid");
    return saveEmergencyProfile(userId, body);
...
async function handle(operation: (userId: string) => Promise<unknown>, scope?: string) {
  try {
    ...
    if (error instanceof EmergencyProfileConflictError) return Response.json({ error: "emergency_profile_conflict", message: error.message }, { status: 409, headers: noStore });
    reportOperationalError("patient.emergency_profile.failed", error);
