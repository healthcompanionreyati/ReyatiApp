...
import { consumeDocumentAccessGrant, DocumentDeliveryError } from "@/lib/document-delivery";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await enforceWriteRateLimit(user.id, "documents.access.consume", { limit: 30 });
    const body = await boundedBody(request);
    const content = await consumeDocumentAccessGrant(user.id, body);
    return new Response(content.body, { headers: content.headers });
  } catch (error) {
    ...
    reportOperationalError("documents.content_delivery_failed", error);
