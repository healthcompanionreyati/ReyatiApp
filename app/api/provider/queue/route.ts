...
import { DigitalQueueConflictError, DigitalQueueValidationError, getProviderDigitalQueue, updateProviderDigitalQueue } from "@/lib/digital-queue";

export const dynamic = "force-dynamic";
const noStore = { "Cache-Control": "private, no-store" };
export async function GET() { return handle(userId => getProviderDigitalQueue(userId)); }
export async function POST(request: Request) { return handle(async userId => {
  ...
  return updateProviderDigitalQueue(userId, body);
}, "provider.digital-queue"); }
async function handle(operation: (userId: string) => Promise<unknown>, scope?: string) {
  try {
    ...
    if (scope) await enforceWriteRateLimit(user.id, scope, { limit: 90 });
    ...
    reportOperationalError("provider.digital_queue.failed", error);
