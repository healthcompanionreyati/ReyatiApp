...
export default function Notifications() {
  ...
  return <main className={`notification-shell ${ar ? "arabic" : ""}`} dir={ar ? "rtl" : "ltr"} id="main-content">
    <PatientHeader ar={ar} displayName={ar ? "عضو رعايتي" : "Reyati member"} onLocaleChange={() => setLang(ar ? "en" : "ar")} active="messages" />
    ...
    <section className="notification-hero"><div><p>{ar ? "مركز الاتصال" : "COMMUNICATION CENTRE"}</p><h1>{ar ? "الإشعارات" : "Notifications"}</h1><span>{ar ? "تحديثات مملوكة لحسابك من نشاطك الحقيقي في رعايتي، مع معاينات تحمي الخصوصية." : "Account-owned updates from your real Reyati activity, with privacy-safe previews."}</span></div>{Boolean(data?.unreadCount) && <button onClick={() => void markAll()}>✓ {ar ? "تحديد الكل كمقروء" : "Mark all as read"}</button>}</section>
