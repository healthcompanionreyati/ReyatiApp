...
async function api(url: string, init?: RequestInit) {
  const response = await fetch(url, init); const payload = await response.json() as { data?: unknown; message?: string; error?: string };
...
export default function Notifications() {
  ...
  useEffect(() => { let active = true; api("/api/notifications").then((next) => { if (active) { const inbox = next as InboxData; setData(inbox); setItems(inbox.notifications); } }).catch((caught) => { if (active) setError(caught instanceof Error ? caught.message : "Notifications unavailable"); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
