...
export default function ProviderDocumentsPage() {
  ...
  const [documents, setDocuments] = useState<SharedDocument[]>([]); const [contentAccessEnabled, setContentAccessEnabled] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [downloading, setDownloading] = useState("");
  const load = useCallback(async (signal?: AbortSignal) => {
    setLoading(true); setError("");
    try {
      const response = await fetch("/api/provider/documents", { cache: "no-store", signal });
      if (response.status === 401) { window.location.assign("/sign-in?redirect_url=/provider/documents"); return; }
      const payload = await response.json().catch(() => ({})) as { data?: { documents: SharedDocument[]; contentAccessEnabled: boolean } };
      ...
      setDocuments(payload.data.documents); setContentAccessEnabled(payload.data.contentAccessEnabled);
    } catch (caught) {
  ...
  async function download(document: SharedDocument) {
    setDownloading(document.documentId); setError("");
    try {
      const grantResponse = await fetch("/api/documents/access", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentId: document.documentId }) });
      ...
      const extension = document.contentType === "application/pdf" ? "pdf" : document.contentType === "image/jpeg" ? "jpg" : "png";
      const url = URL.createObjectURL(await contentResponse.blob()); const anchor = window.document.createElement("a");
      anchor.href = url; anchor.download = `qivaya-shared-${document.category}.${extension}`; anchor.click();
  ...
  return <main className={`provider-shell provider-live-shell ${ar ? "arabic" : ""}`} dir={ar ? "rtl" : "ltr"} id="main-content">
    ...
    <section className="provider-main"><header className="provider-topbar"><div className="provider-context"><span>▤</span><div><b>{ar ? "المستندات الطبية المشتركة" : "Shared medical documents"}</b><small>{ar ? "بيانات محددة بالموافقة" : "Consent-scoped metadata"}</small></div></div><div className="provider-actions"><button className="lang" type="button" onClick={() => setLang(ar ? "en" : "ar")}>{ar ? "English" : "العربية"}</button><a href="/notifications">{ar ? "الإشعارات" : "Notifications"}</a><span className="provider-avatar">PR</span></div></header>
      <div className="provider-workspace provider-documents-workspace"><div className="provider-welcome"><div><p>{ar ? "وصول مصرح به من المريض" : "Patient-authorized access"}</p><h1>{ar ? "المستندات المشتركة" : "Shared documents"}</h1><span>{ar ? "اعرض فقط بيانات المستند النشطة والمحددة زمنياً التي منحها مرضى مرتبطون بمواعيدك." : "See only active, time-limited document metadata granted by appointment-linked patients."}</span></div><button onClick={() => void load()}>{ar ? "تحديث" : "Refresh"}</button></div>
        <div className={`provider-document-boundary ${contentAccessEnabled ? "active" : ""}`}><span>{contentAccessEnabled ? "✓" : "i"}</span><div><b>{contentAccessEnabled ? (ar ? "تسليم المستند المحمي نشط." : "Protected document delivery is active.") : (ar ? "الوصول إلى البيانات الوصفية فقط." : "Metadata-only access is active.")}</b><p>{contentAccessEnabled ? (ar ? "يتطلب كل تنزيل منحة قصيرة العمر، ويعاد التحقق من الموافقة والدور قبل التسليم." : "Every download requires a short-lived grant, with consent and role rechecked before delivery.") : (ar ? "ستظهر أزرار التنزيل تلقائياً عند تفعيل بوابة التسليم الخاصة." : "Download controls will appear automatically when private delivery is activated.")}</p></div></div>
        {error && <div className="provider-live-error"><span>{error}</span><button onClick={() => void load()}>{ar ? "حاول مرة أخرى" : "Try again"}</button></div>}
        {loading ? <div className="provider-live-state"><span>◌</span><h2>{ar ? "جارٍ تحميل منح الوصول النشطة" : "Loading active access grants"}</h2><p>{ar ? "جارٍ التحقق من الانتهاء والموافقة ونطاق مقدم الرعاية." : "Checking expiry, consent, and provider scope."}</p></div> : documents.length === 0 ? <div className="provider-live-card provider-live-state"><span>▤</span><h2>{ar ? "لا توجد مستندات مشتركة معك" : "No documents shared with you"}</h2><p>{ar ? "ستظهر بيانات المستند المعتمدة من المريض هنا أثناء سريان منحة الوصول." : "Patient-approved document metadata will appear here while its access grant is active."}</p></div> : <section className="provider-shared-documents">{documents.map((document) => <article key={document.shareId}><span className="document-icon">▤</span><div><p>{label(document.category)}</p><h2>{document.patientName}</h2><span>{document.pageCount ? `${document.pageCount} ${ar ? "صفحة" : "pages"} · ` : ""}{label(document.verificationStatus)} · {ar ? "التقاط" : "Captured"} {date(document.capturedAt, lang)}</span><small>{label(document.purpose)} · {ar ? "ينتهي الوصول" : "Access expires"} {date(document.expiresAt, lang)}</small></div>{contentAccessEnabled ? <button type="button" className="provider-document-download" disabled={downloading === document.documentId} onClick={() => void download(document)}>{downloading === document.documentId ? (ar ? "جارٍ الفتح…" : "Opening…") : (ar ? "تنزيل آمن" : "Secure download")}</button> : <span className="documents-status ready">{ar ? "بيانات فقط" : "Metadata only"}</span>}</article>)}</section>}
      </div>
    </section>
  </main>;
}
