...
export default function AccessibilitySettingsPage() {
  ...
  return <main className={`${styles.shell} trust-center-shell accessibility-settings-experience`} dir={ar ? "rtl" : "ltr"}>
    <header className={styles.top}><a href="/"><img src="/brand/qivaya-logo-primary.png" alt="Qivaya" /></a><nav aria-label={ar ? "التنقل الرئيسي" : "Primary navigation"}><a href="/account/security">{ar ? "أمان الحساب" : "Account security"}</a><a href="/notification-preferences">{ar ? "الإشعارات" : "Notifications"}</a><button type="button" onClick={() => setLang(ar ? "en" : "ar")}>{ar ? "English" : "العربية"}</button></nav></header>
    <section className={styles.hero}><div><span>{ar ? "تجربة مصممة لك" : "AN EXPERIENCE SHAPED BY YOU"}</span><h1>{ar ? "اللغة وإمكانية الوصول" : "Language & accessibility"}</h1><p>{ar ? "احفظ الطريقة التي تفضل بها قراءة واستخدام كيفايا وطلب المساعدة. تبقى اختياراتك تحت سيطرتك." : "Save how you prefer to read, navigate, and ask for help in Qivaya. Your choices stay under your control."}</p></div><aside><i aria-hidden="true">Aa</i><b>{ar ? "اختيارات واضحة" : "Clear choices"}</b><small>{ar ? "دون افتراض احتياجاتك" : "No inferred needs"}</small></aside></section>
      ...
      {loading ? <div className={styles.loading} role="status" aria-live="polite"><i /><p>{ar ? "جارٍ تحميل إعداداتك…" : "Loading your settings…"}</p></div> : data && <>
        <div className={styles.layout}><div className={styles.stack}>
          ...
          <section className={styles.panel}><div className={styles.panelHead}><div><span>{ar ? "المساعدة والتفاعل" : "ASSISTANCE & INTERACTION"}</span><h2>{ar ? "احفظ تفضيلات استخدامك" : "Save your interaction preferences"}</h2></div></div><div className={styles.choices}>
            ...
            <Choice label={ar ? "مساعدة قارئ الشاشة" : "Screen-reader assistance"} description={ar ? "أخبر دعم كيفايا أنك تفضل المساعدة المتوافقة مع قارئ الشاشة." : "Tell Qivaya support you prefer screen-reader-compatible assistance."} value={form.screenReaderAssistance} onClick={() => toggle("screenReaderAssistance")} ar={ar} />
