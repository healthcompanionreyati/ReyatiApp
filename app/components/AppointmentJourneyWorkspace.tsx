...
const copy = {
  ...
  care_timeline: { en: "Care journey timeline", ar: "الخط الزمني لرحلة الرعاية", introEn: "A source-labelled history of your Qivaya appointment journey.", introAr: "سجل موضح المصدر لرحلة مواعيدك في ريّاتي." },
} as const;
...
export default function AppointmentJourneyWorkspace({ module, role }: { module: Module; role: Role }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const response = await fetch(endpoint, { cache: "no-store" }); if (response.status === 401) { window.location.assign(`/sign-in?redirect_url=${encodeURIComponent(location.pathname)}`); return; } const payload = await response.json().catch(() => ({})) as { data?: Workspace; message?: string }; if (!response.ok || !payload.data) throw new Error(payload.message || (ar ? "تعذر تحميل مساحة العمل." : "Unable to load this workspace.")); setData(payload.data); } catch (caught) { setError(caught instanceof Error ? caught.message : (ar ? "تعذر التحميل." : "Unable to load.")); } finally { setLoading(false); } }, [endpoint, ar]);
  ...
  return <main className={`${styles.shell} ${role === "patient" && module === "care_timeline" ? "health-hub-shell care-timeline-experience" : ""}`} dir={ar ? "rtl" : "ltr"}>
    <header className={styles.top}><a href={role === "provider" ? "/provider" : "/"}><img src="/brand/qivaya-logo-primary.png" alt="Qivaya"/></a><div className={styles.topActions}><button type="button" className={styles.lang} onClick={() => setLang(ar ? "en" : "ar")}>{ar ? "EN" : "العربية"}</button><a href={role === "provider" ? "/provider" : "/appointments"}>{ar ? "رجوع" : "Back"}</a></div></header>
