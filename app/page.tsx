...
export default function Home(){
  ...
  useEffect(()=>{let active=true;Promise.all([fetch("/api/me",{cache:"no-store"}),fetch("/api/appointments",{cache:"no-store"})]).then(async([identityResponse,appointmentResponse])=>{
    if(identityResponse.status===401||appointmentResponse.status===401){window.location.assign(`/signin-with-chatgpt?return_to=${encodeURIComponent("/")}`);return null;}
    const identity=await identityResponse.json() as {user?:User;error?:string};const schedule=await appointmentResponse.json() as {appointments?:Appointment[];error?:string};
    if(!identityResponse.ok)throw new Error(identity.error||"Your Reyati identity is temporarily unavailable");if(!appointmentResponse.ok)throw new Error(schedule.error||"Appointments are temporarily unavailable");return{user:identity.user||null,appointments:schedule.appointments||[]};
  }).then((result)=>{if(active&&result){setUser(result.user);setAppointments(result.appointments);}}).catch((caught)=>{if(active)setError(caught instanceof Error?caught.message:"Reyati is temporarily unavailable");}).finally(()=>{if(active)setLoading(false);});return()=>{active=false};},[]);
  ...
  return <main id="main-content">
    <header><a className="brand" href="/" aria-label="Reyati home"><img src="/brand/reyati-logo.svg" alt="Reyati"/></a><nav><a className="active" href="/">Home</a><a href="/providers">Find care</a><a href="/appointments">Appointments</a><a href="/wallet">Health records</a><a href="/family">Family access</a><a href="/support">Support</a></nav><div className="header-actions"><a className="bell" href="/notifications" aria-label="Notifications">●</a><a className="account-trigger" href="/auth" aria-label="Open secure account"><span className="avatar">{initials(displayName)}</span><span className="profile">Account</span></a></div></header>
    <section className="hero"><div className="hero-inner"><p className="eyebrow">CARE, INTELLIGENTLY CONNECTED</p><h1>{loading?"Welcome to Reyati":`Welcome, ${displayName}`}</h1><p className="lead">Manage real appointments, finalized visit information, scoped family access, and support from one secure account.</p><div className="home-hero-actions"><a className="search-button" href="/providers">Find verified care</a><a href="/appointments">View appointments</a></div><p className="emergency"><span>＋</span>For a life-threatening emergency in Qatar, call 999.</p></div><div className="hero-art" aria-hidden="true"><img src="/brand/care-conversation.png" alt=""/><span className="weave-path weave-cyan"/><span className="weave-path weave-white"/></div></section>
