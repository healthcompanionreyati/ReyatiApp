...
type FinanceData = {
  ...
  metrics: {
    ...
    processorEventsReceived: number;
    processorEventsFailed: number;
  };
  statuses: StatusRow[];
  paymentProvider: { provider: "stripe"; enabled: boolean; mode: "test" | "live" | null; checkoutReady: boolean; webhookReady: boolean; reason: string | null };
  processorEvents: { status: string; count: number }[];
};
...
export default function FinanceOperations() {
  ...
  function exportAggregate() {
    if (!data) return;
    const rows = [
      ["Qivaya payment ledger aggregate"],
  ...
  return <main className={`finance-shell live-finance-shell ${ar ? "arabic" : ""}`} dir={ar ? "rtl" : "ltr"}>
    ...
    <section className="finance-main"><header className="finance-top"><div><span>{ar ? "دفتر مسجل" : "RECORDED LEDGER"}</span><b>{ar ? "وصول إداري محمي للقراءة فقط" : "Protected, read-only administrator access"}</b></div><div><button type="button" onClick={() => setLang(ar ? "en" : "ar")}>{ar ? "English" : "العربية"}</button><a href="/notifications" aria-label={ar ? "الإشعارات" : "Notifications"}>●</a><span>{avatar}</span></div></header><div className="finance-workspace">
      ...
      <div className="finance-banner"><span>▣</span><p><b>{ar ? "القيمة المسجلة لا تعني أن المال تحرك" : "Recorded value does not prove money moved"}</b>{ar ? "ينشئ الحجز إدخالاً بحالة «غير محصّل» وبقيمة الخدمة المنشورة. لا تُعتبر القيمة مدفوعة إلا بعد حدث مزود موقّع." : "Booking creates a not-charged entry using the published service fee. Value is counted as paid only after a signed provider event."}</p><i>{data?.paymentProvider.enabled ? `${data.paymentProvider.provider.toUpperCase()} · ${data.paymentProvider.mode?.toUpperCase()}` : (ar ? "المزود غير نشط" : "PROVIDER INACTIVE")}</i></div>
      ...
      {loading && !data ? <section className="finance-live-state" aria-live="polite"><span>◌</span><h2>{ar ? "جارٍ تحميل دفتر المدفوعات" : "Loading payment ledger"}</h2><p>{ar ? "يتم التحقق من دور مسؤول المنصة." : "Verifying the platform administrator role."}</p></section> : error ? <section className="finance-live-state" role="alert"><span>!</span><h2>{error === "auth" ? (ar ? "يلزم تسجيل الدخول" : "Sign in required") : error === "forbidden" ? (ar ? "دور مسؤول المنصة مطلوب" : "Platform administrator access required") : (ar ? "تعذر تحميل الدفتر" : "Ledger could not be loaded")}</h2><p>{error === "forbidden" ? (ar ? "يجب تعيين دور مسؤول منصة نشط لهذا الحساب." : "This account must have an active platform administrator role.") : (ar ? "أعد المحاولة أو افتح الدعم." : "Try again or open support if the problem continues.")}</p><a href={error === "auth" ? "/auth" : error === "forbidden" ? "/admin/access" : "/support"}>{error === "auth" ? (ar ? "تسجيل الدخول" : "Sign in") : error === "forbidden" ? (ar ? "مراجعة الوصول" : "Review access") : (ar ? "فتح الدعم" : "Open support")}</a></section> : data && <>
        ...
        <section className="finance-boundaries"><article><span>↔</span><div><h2>{ar ? "تأكيد المزود والمطابقة" : "Provider confirmation and reconciliation"}</h2><p>{data.paymentProvider.enabled ? (ar ? "تُعتمد تغييرات الدفتر من أحداث Stripe الموقّعة. تبقى مطابقة كشف البنك والتسوية خارج التطبيق." : "Ledger changes are accepted from signed Stripe events. Bank-statement reconciliation and settlement remain outside the application.") : (ar ? "بوابة الدفع غير مفعلة. لا توجد مطابقة كشف بنك أو حساب مستحقات لمقدم الخدمة." : "The payment gateway is inactive. No bank-statement or provider-payable reconciliation is connected.")}</p><small>{data.metrics.processorEventsReceived} {ar ? "حدث مزود مسجل" : "provider events recorded"} · {data.metrics.processorEventsFailed} {ar ? "فشل معالجة" : "processing failures"}</small></div><b>{data.paymentProvider.webhookReady ? (ar ? "التوقيع جاهز" : "Signed webhook ready") : (ar ? "غير نشط" : "Inactive")}</b></article><article><span>↩</span><div><h2>{ar ? "تنفيذ الاسترداد" : "Refund execution"}</h2><p>{ar ? "يمكن للدفتر تسجيل استرداد أكده المزود، لكنه لا يبدأ إرسال الأموال تلقائياً." : "The ledger can record a provider-confirmed refund, but Qivaya does not initiate money movement automatically."}</p></div><b>{data.metrics.pendingRefundEntries ? `${data.metrics.pendingRefundEntries} ${ar ? "معلّق" : "recorded pending"}` : (ar ? "لا يوجد معلّق" : "None pending")}</b></article></section>
