...
export default function CommunicationOperations() {
  ...
  return <main className="comms-ops-shell" id="main-content"><aside className="comms-ops-sidebar"><a href="/" className="provider-logo"><img src="/brand/reyati-logo-reversed.svg" alt="Reyati"/><span>Platform operations</span></a><div className="comms-ops-role"><span>{avatar}</span><div><b>{data?.operatorName ?? "Communications operator"}</b><small>{data ? label(data.role) : "Checking access"}</small></div></div><nav><a href="/admin">Overview</a><a href="/admin/cases">Support cases</a><a className="active" href="/admin/communications">Communications</a><a href="/admin/audit">Audit ledger</a></nav><div className="comms-ops-side-note"><span>▣</span><p><b>Privacy-minimized view</b>No recipient address, message body, invitation token, or webhook payload is displayed here.</p></div></aside>
