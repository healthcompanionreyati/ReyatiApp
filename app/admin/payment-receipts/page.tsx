...
type Credit = { id: string; creditNoteNumber: string; amountMinor: number; currency: string; issuedAt: string; artifact: Artifact | null; emailDelivery: Delivery | null };
type Receipt = { id: string; receiptNumber: string; providerName: string; facilityName: string | null; appointmentStartedAt: string; careMode: string; amountMinor: number; currency: string; issuedAt: string; artifact: Artifact | null; emailDelivery: Delivery | null; creditNotes: Credit[] };
type Data = {
  generatedAt: string;
  ...
  metrics: { receiptCount: number; receiptAmountMinor: number; creditNoteCount: number; creditAmountMinor: number; deliveryTracked: number; deliveryCompleted: number; deliveryAttention: number; pdfReady: number; pdfPending: number; pdfAttention: number };
  receipts: Receipt[];
};
...
function artifact(value: Artifact | null, ar: boolean) { return value?.status === "ready" ? (ar ? "PDF خاص جاهز" : "Private PDF ready") : value ? (ar ? "PDF يحتاج انتباهاً" : "PDF needs attention") : (ar ? "بانتظار الإنشاء" : "Pending generation"); }
...
export default function AdminPaymentReceipts() {
  ...
  return <main className={styles.shell} dir={ar ? "rtl" : "ltr"}>
    <aside className={styles.side}><a href="/admin"><Image src="/brand/qivaya-logo-reversed.png" width={116} height={44} alt="Qivaya" /></a><span>{ar ? "عمليات المنصة" : "PLATFORM OPERATIONS"}</span><nav><a href="/admin/finance">{ar ? "الدفتر المالي" : "Finance ledger"}</a><a href="/admin/finance-controls">{ar ? "ضوابط الاسترداد" : "Refund controls"}</a><a className={styles.active} href="/admin/payment-receipts">{ar ? "الإيصالات" : "Receipts"}</a><a href="/admin/payment-reconciliation">{ar ? "المطابقة" : "Reconciliation"}</a><a href="/admin/payment-disputes">{ar ? "النزاعات" : "Disputes"}</a><a href="/admin/payment-lifecycle-rehearsal">{ar ? "بروفة دورة الدفع" : "Lifecycle rehearsal"}</a></nav><div className={styles.boundary}><b>{ar ? "سجل للقراءة فقط" : "Read-only register"}</b><p>{ar ? "لا ينشئ المستند حركة أموال أو تسوية أو مطالبة ضريبية." : "Documents create no money movement, settlement, or tax claim."}</p></div></aside>
    <section className={styles.workspace}><header className={styles.top}><a href="/admin/finance">← {ar ? "المالية" : "Finance"}</a><div><span>{ar ? "دليل مزود غير قابل للتغيير" : "IMMUTABLE PROVIDER EVIDENCE"}</span><button onClick={() => setLang(ar ? "en" : "ar")}>{ar ? "English" : "العربية"}</button></div></header><div className={styles.content}>
      ...
      {data && <section className={deliveryStyles.deliverySummary}><div><span>PDF</span><p><b>{ar ? "أرشيف R2 الخاص" : "Private R2 archive"}</b>{ar ? "ملفات مولدة من الخادم مع فحص سلامة وتنزيل لمرة واحدة." : "Server-generated artifacts with integrity verification and one-time download access."}</p></div><dl><div><dt>{ar ? "جاهز" : "Ready"}</dt><dd>{data.metrics.pdfReady}</dd></div><div><dt>{ar ? "معلّق" : "Pending"}</dt><dd>{data.metrics.pdfPending}</dd></div><div><dt>{ar ? "انتباه" : "Attention"}</dt><dd>{data.metrics.pdfAttention}</dd></div></dl><a href="/admin/retention-automation">{ar ? "فتح دورة الحياة" : "Open lifecycle controls"} →</a></section>}
      ...
      {loading ? <div className={styles.state}>{ar ? "جارٍ تحميل سجل الإيصالات…" : "Loading receipt register…"}</div> : !visible.length ? <div className={styles.state}><b>{ar ? "لا توجد إيصالات مطابقة" : "No matching receipts"}</b><span>{ar ? "تظهر المستندات فقط بعد تأكيد مزود موقّع." : "Documents appear only after signed provider confirmation."}</span></div> : <div className={styles.grid}>
        <aside className={styles.list}>{visible.map((item) => <button key={item.id} className={receipt?.id === item.id ? styles.chosen : ""} onClick={() => setSelected(item.id)}><span><b>{item.providerName}</b><small>{item.receiptNumber}</small></span><span><strong>{money(item.amountMinor, item.currency, ar)}</strong><small>{delivery(item.emailDelivery, ar)} · {artifact(item.artifact, ar)}</small></span></button>)}</aside>
        <section className={styles.detail}>{receipt && <><header><div><p>{ar ? "إيصال مؤكد" : "CONFIRMED RECEIPT"}</p><h2>{receipt.receiptNumber}</h2><span>{receipt.providerName}</span></div><em>{ar ? "غير قابل للتغيير" : "Immutable"}</em></header><dl><div><dt>{ar ? "القيمة" : "Value"}</dt><dd>{money(receipt.amountMinor, receipt.currency, ar)}</dd></div><div><dt>{ar ? "الموقع" : "Facility"}</dt><dd>{receipt.facilityName || "—"}</dd></div><div><dt>{ar ? "موعد الرعاية" : "Appointment"}</dt><dd>{stamp(receipt.appointmentStartedAt, ar)}</dd></div><div><dt>{ar ? "تاريخ الإصدار" : "Issued"}</dt><dd>{stamp(receipt.issuedAt, ar)}</dd></div><div><dt>{ar ? "طريقة الرعاية" : "Care mode"}</dt><dd>{receipt.careMode.replaceAll("_", " ")}</dd></div><div><dt>{ar ? "تسليم البريد" : "Email delivery"}</dt><dd>{delivery(receipt.emailDelivery, ar)}</dd></div><div><dt>{ar ? "أرشيف PDF" : "PDF archive"}</dt><dd>{artifact(receipt.artifact, ar)}</dd></div><div><dt>{ar ? "هوية المريض" : "Patient identity"}</dt><dd>{ar ? "غير معروضة" : "Not exposed"}</dd></div></dl><section className={styles.credits}><h3>{ar ? "الإشعارات الدائنة" : "Credit notes"}</h3>{receipt.creditNotes.length ? receipt.creditNotes.map((note) => <article key={note.id}><div><b>{note.creditNoteNumber}</b><small>{stamp(note.issuedAt, ar)} · {delivery(note.emailDelivery, ar)} · {artifact(note.artifact, ar)}</small></div><strong>−{money(note.amountMinor, note.currency, ar)}</strong></article>) : <p>{ar ? "لا توجد إشعارات دائنة" : "No credit notes"}</p>}</section><div className={styles.notice}>{ar ? "سجل التطبيق هو المرجع. ملفات PDF خاصة وتخضع للوصول لمرة واحدة ودورة حياة المستند؛ ولا تعد فواتير ضريبية." : "The in-app record is authoritative. PDFs remain private, one-time-access lifecycle records—not tax invoices."}</div></>}</section>
      </div>}
    </div></section>
  </main>;
}
