...
export default function VerificationWorkspace() {
  ...
  return <main className={`verify-shell ${ar ? "arabic" : ""}`} dir={ar ? "rtl" : "ltr"}>
    ...
    <ConfirmActionDialog open={Boolean(selected && confirmReject)} title={ar ? "رفض طلب التحقق؟" : `Reject ${selected?.providerName ?? "this provider"}?`} description={ar ? "سيتم تسجيل ملاحظة القرار وإزالة مقدم الرعاية من قائمة التحقق." : "The decision note will be recorded and the provider will leave the verification queue."} consequence={ar ? "سيتم سحب النشر. يجب على مقدم الرعاية إعادة التقديم قبل أن يمكن التحقق منه لاحقًا." : "Publication will be withdrawn. The provider must submit again before they can be verified later."} confirmLabel={ar ? "رفض التحقق" : "Reject verification"} busyLabel={ar ? "جارٍ الرفض…" : "Rejecting…"} busy={saving} onCancel={() => setConfirmReject(false)} onConfirm={() => void decide("rejected")}/>
  </main>;
}
