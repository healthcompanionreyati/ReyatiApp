...
export async function transitionCarePlan(userId: string, bodyValue: unknown) {
  ...
  await db.batch([
    ...
    ...(patientUser ? [db.insert(notifications).values(notificationRecord({ userId: patientUser.userId, type: "care_plan_update", title: "Care plan updated", body: "Your provider updated the status or instructions of your appointment-linked care plan. Review the new version in Qivaya.", actionPath: "/care-plan", resourceType: "care_plan", resourceId: planId, dedupeKey: `care-plan:${planId}:v${nextVersion}`, createdAt: now })).onConflictDoNothing({ target: [notifications.userId, notifications.dedupeKey] })] : []),
...
export async function resolveCarePlanReview(userId: string, bodyValue: unknown) {
  ...
  await db.batch([
    ...
    db.insert(notifications).values(notificationRecord({ userId: request.request.patientUserId, type: "care_plan_review", title: "Care plan review completed", body: "Your provider completed the requested care plan review. Open Qivaya to see its status.", actionPath: "/care-plan", resourceType: "care_plan_review_request", resourceId: requestId, dedupeKey: `care-plan-review:${requestId}:resolved`, createdAt: now })).onConflictDoNothing({ target: [notifications.userId, notifications.dedupeKey] }),
