...
import { appointments, auditEvents, authEvents, controlledPilotPlans, dataLifecyclePolicies, documentDeletionJobs, documentRecords, documentUploadSessions, monitoringAcceptanceRuns, observabilityPolicies, observabilityValidationRuns, operationalRateLimits, outboundMessages, pilotControlAssignments, pilotEnrollmentDocuments, pilotInvitationPolicies, pilotParticipationPolicies, pilotSuccessMetrics, pilotWithdrawalDrills, recoveryRehearsals, supportCases, webhookReceipts } from "@/db/schema";
...
export async function getOperationsHealth(userId: string, operatorName: string) {
  ...
  const [ownershipAssignments, rehearsals, lifecyclePolicies, telemetryPolicies, telemetryValidations, monitoringAcceptances, pilotPlans, enrollmentDocuments, invitationPolicies, participationPolicies, withdrawalDrills, successMetrics] = await Promise.all([
    ...
    db.select().from(observabilityValidationRuns), db.select().from(monitoringAcceptanceRuns),
