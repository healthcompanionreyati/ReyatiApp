...
export async function getOperationsHealth(userId: string, operatorName: string) {
  ...
  const [ownershipAssignments, rehearsals, lifecyclePolicies, lifecycleAcceptances, lifecyclePrerequisites, telemetryPolicies, telemetryValidations, monitoringAcceptances, pilotPlans, enrollmentDocuments, invitationPolicies, participationPolicies, withdrawalDrills, successMetrics] = await Promise.all([
  ...
  const pilotReadiness = {
    ...
    gates: [
      ...
      { id: "data_lifecycle", name: "Clinical data lifecycle", status: lifecycleEvidenceReady ? "cleared" as const : "blocked" as const, evidence: lifecycleEvidenceReady ? "All five record-class policies, current retention and legal-hold controls, protected storage, private scanning, scheduled maintenance, and a synthetic-only destructive rehearsal are backed by fresh independently verified production acceptance." : `${approvedLifecyclePolicies}/5 policies are approved; runtime prerequisite status is ${lifecyclePrerequisites.prerequisitesReady ? "complete" : "blocked"}. Fresh independently verified production lifecycle acceptance is required.`, ownerNeeded: !lifecycleOwnershipReady, href: "/admin/data-lifecycle-acceptance" },
    ],
  };
