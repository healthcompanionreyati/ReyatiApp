...
export async function getOperationsHealth(userId: string, operatorName: string) {
  ...
  const controls = [
    ...
    { id: "retention_enforcement", name: "Automated retention enforcement", status: "partial", note: "Approved-plan enforcement, hold/access-aware job creation, leased verified deletion, broad-scope hold release, and hourly scheduling are implemented but remain disabled pending policy approval and destructive-operation rehearsal." },
