...
type ScanPayload = { documentId?: unknown; status?: unknown; providerReference?: unknown; reasonCode?: unknown; occurredAt?: unknown };
...
export async function processDocumentScanWebhook(rawBody: string, headers: Headers) {
  ...
  const document = await db.select({ id: documentRecords.id, objectKey: documentRecords.objectKey, status: documentRecords.status, contentType: documentRecords.contentType, sizeBytes: documentRecords.sizeBytes, version: documentRecords.version, sourceOrganizationId: documentRecords.sourceOrganizationId })
  ...
  if (reportedStatus === "clean") {
    ...
    if (!object || object.size !== document[0].sizeBytes || object.contentType !== document[0].contentType) {
  ...
  await db.batch([
    db.update(documentRecords).set({ status: nextStatus, malwareScanStatus, quarantineReasonCode: nextStatus === "quarantined" ? reasonCode ?? `scanner_${finalStatus}` : null, version: document[0].version + 1, updatedAt: now })
