...
import { auditEvents, medicationReminderSchedulerRunEvents, medicationReminderSchedulerRuns, medicationReminderSchedulerScenarios, medicationReminderSchedulerSuites } from "@/db/schema";
...
export class MedicationReminderReadinessConflictError extends Error { constructor() { super("This scheduler evidence changed. Refresh and try again."); this.name = "MedicationReminderReadinessConflictError"; } }
...
function expectedVersion(value: unknown) { const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < 1) throw new MedicationReminderReadinessValidationError("version is invalid"); return parsed; }
...
export async function getMedicationReminderReadinessCentre(userId: string) {
  ...
  const [suites, scenarios, runs, events] = await Promise.all([db.select().from(medicationReminderSchedulerSuites).orderBy(desc(medicationReminderSchedulerSuites.createdAt)), db.select().from(medicationReminderSchedulerScenarios).orderBy(medicationReminderSchedulerScenarios.scenarioKey), db.select().from(medicationReminderSchedulerRuns).orderBy(desc(medicationReminderSchedulerRuns.executedAt)).limit(100), db.select().from(medicationReminderSchedulerRunEvents).orderBy(desc(medicationReminderSchedulerRunEvents.createdAt)).limit(300)]);
  return { role: access.role, suiteVersion: MEDICATION_REMINDER_SCHEDULER_SUITE_VERSION, occurrenceMaterializationEnabled: foundationFlags.medicationReminderOccurrenceMaterialization, deliveryEnabled: foundationFlags.medicationReminderDelivery, ocrImportEnabled: foundationFlags.medicationReminderOcrImport, suites: suites.map(suite => ({ ...suite, scenarios: scenarios.filter(item => item.suiteId === suite.id).map(item => ({ ...item, input: JSON.parse(item.inputJson) as ScenarioInput })), runs: runs.filter(item => item.suiteId === suite.id).map(item => ({ ...item, failures: JSON.parse(item.failuresJson) as string[], events: events.filter(event => event.runId === item.id) })) })) };
}
...
export async function runMedicationReminderReadinessEvaluation(userId: string, body: Record<string, unknown>) {
  ...
  await db.batch([db.insert(medicationReminderSchedulerRuns).values({ id, suiteId, executedByUserId: userId, totalScenarios: scenarios.length, passedScenarios, failedScenarios: failures.length, duplicateOccurrences, invalidSourceOccurrences, deliveryAttempts, result, failuresJson: JSON.stringify(failures), dataMode: "synthetic_only", status: "pending_review", version: 1, executedAt: now }), db.insert(auditEvents).values({ id: crypto.randomUUID(), actorUserId: userId, organizationId: null, action: "medication_reminder_scheduler.evaluation_run", resourceType: "medication_reminder_scheduler_run", resourceId: id, outcome: result === "pass" ? "success" : "blocked", metadataJson: JSON.stringify({ suiteId, totalScenarios: scenarios.length, passedScenarios, duplicateOccurrences, invalidSourceOccurrences, deliveryAttempts, dataMode: "synthetic_only", reviewStatus: "pending_review", occurrenceMaterializationEnabled: false, clinicalContentIncluded: false }), createdAt: now })]);
  return { id, result, status: "pending_review", version: 1, totalScenarios: scenarios.length, passedScenarios, failedScenarios: failures.length, duplicateOccurrences, invalidSourceOccurrences, deliveryAttempts, occurrenceMaterializationEnabled: false, deliveryEnabled: false };
}

export async function reviewMedicationReminderReadinessRun(userId: string, body: Record<string, unknown>) {
  await requirePlatformRole(userId, ["security_auditor"]); const runId = text(body.runId, "runId", 1, 128), action = text(body.action, "action", 6, 6), note = text(body.note, "note", 20, 1200), version = expectedVersion(body.version); if (!(["verify", "reject"] as string[]).includes(action)) throw new MedicationReminderReadinessValidationError("action is invalid");
  const db = await getDb(), run = (await db.select().from(medicationReminderSchedulerRuns).where(eq(medicationReminderSchedulerRuns.id, runId)).limit(1))[0]; if (!run || run.status !== "pending_review") throw new MedicationReminderReadinessValidationError("A pending scheduler run is required"); if (run.executedByUserId === userId) throw new MedicationReminderReadinessValidationError("The evaluation operator cannot independently review the same run"); if (action === "verify" && run.result !== "pass") throw new MedicationReminderReadinessValidationError("A failed evaluation cannot be verified");
  const nextStatus = action === "verify" ? "verified" : "rejected", now = new Date(), changed = await db.update(medicationReminderSchedulerRuns).set({ status: nextStatus, reviewerUserId: userId, reviewedAt: now, reviewNote: note, version: version + 1 }).where(and(eq(medicationReminderSchedulerRuns.id, runId), eq(medicationReminderSchedulerRuns.status, "pending_review"), eq(medicationReminderSchedulerRuns.version, version))).returning({ version: medicationReminderSchedulerRuns.version }); if (!changed[0]) throw new MedicationReminderReadinessConflictError();
  await db.batch([db.insert(medicationReminderSchedulerRunEvents).values({ id: crypto.randomUUID(), runId, actorUserId: userId, action, previousStatus: "pending_review", nextStatus, note, createdAt: now }), db.insert(auditEvents).values({ id: crypto.randomUUID(), actorUserId: userId, organizationId: null, action: `medication_reminder_scheduler.${action}`, resourceType: "medication_reminder_scheduler_run", resourceId: runId, outcome: action === "verify" ? "success" : "blocked", metadataJson: JSON.stringify({ previousStatus: "pending_review", nextStatus, result: run.result, dataMode: "synthetic_only", independentReviewer: true, occurrenceMaterializationEnabled: false, deliveryEnabled: false, clinicalContentIncluded: false }), createdAt: now })]);
  return { runId, status: nextStatus, version: changed[0].version, occurrenceMaterializationEnabled: false, deliveryEnabled: false };
}
