...
async function liveSignals(now = new Date()) {
...
export async function getDocumentIncidentWorkspace(userId: string) {
  ...
  const [commands, incidents, events, people, signals, posture] = await Promise.all([
    ...
    db.select().from(documentIncidentEvents).orderBy(desc(documentIncidentEvents.createdAt)).limit(400), roster(), liveSignals(), getDataLifecycleRuntimePosture(),
  ]);
...
function recoveredSignalIsClear(signalCode: string, signals: Awaited<ReturnType<typeof liveSignals>>, posture: Awaited<ReturnType<typeof getDataLifecycleRuntimePosture>>) {
...
export async function reviewDocumentIncidentRecovery(userId: string, body: Record<string, unknown>) {
  ...
  if (current.status !== "recovery_review" || current.version !== expected || !current.recoveryPreparedByUserId) throw new DocumentIncidentConflictError("Only the current prepared recovery can be reviewed"); if (current.recoveryPreparedByUserId === userId || current.openedByUserId === userId) throw new DocumentIncidentConflictError("Recovery closure must be independently reviewed by a different authorized person"); const [signals, posture] = await Promise.all([liveSignals(), getDataLifecycleRuntimePosture()]);
