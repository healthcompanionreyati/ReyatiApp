...
import { auditEvents, dataLifecycleAcceptanceEvents, dataLifecycleAcceptanceRuns, dataLifecyclePolicies, legalHoldOrders, notifications, platformRoles, retentionAutomationPlans, retentionSafetyRehearsals, users } from "@/db/schema";
...
export async function getDataLifecycleAcceptancePrerequisites(now = new Date()) {
  ...
  const [policies, plans, rehearsals, overdueHolds, posture] = await Promise.all([
  ...
  const prerequisitesReady = approvedPolicyCount === requiredRecordClasses.length && approvedRetentionPlan && freshSafetyRehearsal && overdueLegalHoldCount === 0 && posture.productionEnvironment && posture.protectedStorageConfigured && posture.privateScannerConfigured && posture.allRuntimeControlsEnabled;
  return { approvedPolicyCount, requiredPolicyCount: requiredRecordClasses.length, approvedRetentionPlan, freshSafetyRehearsal, safetyScenarioCount: rehearsal?.scenarioCount ?? 0, overdueLegalHoldCount, evidenceWindowDays: EVIDENCE_WINDOW_DAYS, posture, prerequisitesReady };
}
...
export async function createDataLifecycleAcceptance(userId: string, body: Record<string, unknown>) {
  ...
  if (!prerequisites.prerequisitesReady) throw new DataLifecycleAcceptanceValidationError("All lifecycle policies, independent retention approval, fresh safety evidence, legal-hold reviews, protected storage, private scanning, and runtime controls must be ready in production");
