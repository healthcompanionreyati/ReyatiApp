...
import { DocumentPreflightConflictError, DocumentPreflightValidationError } from "@/lib/document-preflight-suite";
...
export async function handleDocumentPreflightRoute(operation: (userId: string) => Promise<unknown>, scope: string, write = false) {
  try {
    ...
    if (error instanceof DocumentPreflightValidationError) return Response.json({ error: "invalid_request", message: error.message }, { status: 400, headers });
    if (error instanceof DocumentPreflightConflictError) return Response.json({ error: "conflict", message: error.message }, { status: 409, headers });
...
export async function documentPreflightJson(request: Request) {
  try { return await request.json() as Record<string, unknown>; }
  catch { throw new DocumentPreflightValidationError("A valid JSON body is required"); }
}
