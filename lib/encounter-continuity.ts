...
export async function createEncounterFollowUp(userId: string, bodyValue: unknown) {
  ...
  await db.batch([
    ...
    db.insert(notifications).values(notificationRecord({ userId: owned.patientUserId, type: "follow_up", title: "New follow-up recommendation", body: "Your provider added a follow-up recommendation. Open Reyati to review the protected instructions and due window.", actionPath: "/encounter-follow-up", resourceType: "follow_up_task", resourceId: id, dedupeKey: `encounter-follow-up:${id}:patient`, createdAt: now })).onConflictDoNothing({ target: [notifications.userId, notifications.dedupeKey] }),
