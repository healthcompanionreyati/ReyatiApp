...
export const paymentAcceptanceRuns = sqliteTable("payment_acceptance_runs", {
  id: text("id").primaryKey(),
  requestedByUserId: text("requested_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }),
  clientRequestId: text("client_request_id").notNull(),
  suiteVersion: text("suite_version").notNull(),
  ...
  providerReadCount: integer("provider_read_count").notNull().default(0),
  moneyMovementMinor: integer("money_movement_minor").notNull().default(0),
  sideEffectsExecuted: integer("side_effects_executed", { mode: "boolean" }).notNull().default(false),
  ...
  uniqueIndex("idx_payment_acceptance_request").on(table.requestedByUserId, table.clientRequestId),
...
export const paymentActivationAssuranceRuns = sqliteTable("payment_activation_assurance_runs", {
  id: text("id").primaryKey(),
  activationWindowId: text("activation_window_id").notNull().references(() => paymentActivationWindows.id, { onDelete: "restrict" }),
  ...
  minimumObservationEndedAt: integer("minimum_observation_ended_at", { mode: "timestamp_ms" }).notNull(),
  status: text("status").notNull(),
  checkCount: integer("check_count").notNull(),
  passedChecks: integer("passed_checks").notNull(),
  failedChecks: integer("failed_checks").notNull(),
  checkResultsJson: text("check_results_json").notNull(),
  processorEventCount: integer("processor_event_count").notNull().default(0),
  failedProcessorEventCount: integer("failed_processor_event_count").notNull().default(0),
  staleProcessorEventCount: integer("stale_processor_event_count").notNull().default(0),
  refundExecutionCount: integer("refund_execution_count").notNull().default(0),
  failedRefundExecutionCount: integer("failed_refund_execution_count").notNull().default(0),
  reconciliationRunId: text("reconciliation_run_id").references(() => paymentReconciliationRuns.id, { onDelete: "restrict" }),
  decision: text("decision").notNull().default("pending"),
  reviewedByUserId: text("reviewed_by_user_id").references(() => users.id, { onDelete: "restrict" }),
  reviewNote: text("review_note"),
  reviewedAt: integer("reviewed_at", { mode: "timestamp_ms" }),
  containmentVerifiedByUserId: text("containment_verified_by_user_id").references(() => users.id, { onDelete: "restrict" }),
  containmentVerifiedAt: integer("containment_verified_at", { mode: "timestamp_ms" }),
  configurationReadCount: integer("configuration_read_count").notNull().default(1),
  stripeCallsMade: integer("stripe_calls_made").notNull().default(0),
  moneyMovementMinor: integer("money_movement_minor").notNull().default(0),
  operationalChangesExecuted: integer("operational_changes_executed", { mode: "boolean" }).notNull().default(false),
  version: integer("version").notNull().default(1),
  ...
  uniqueIndex("idx_payment_assurance_request").on(table.collectedByUserId, table.clientRequestId),
  index("idx_payment_assurance_window_collected").on(table.activationWindowId, table.collectedAt),
  index("idx_payment_assurance_status_decision").on(table.status, table.decision, table.collectedAt),
]);

export const paymentActivationAssuranceEvents = sqliteTable("payment_activation_assurance_events", {
  id: text("id").primaryKey(),
  assuranceRunId: text("assurance_run_id").notNull().references(() => paymentActivationAssuranceRuns.id, { onDelete: "restrict" }),
  actorUserId: text("actor_user_id").notNull().references(() => users.id, { onDelete: "restrict" }),
  eventCode: text("event_code").notNull(),
