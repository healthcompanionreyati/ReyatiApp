...
export const reportReaderSuites = sqliteTable("report_reader_suites", {
  id: text("id").primaryKey(), suiteVersion: text("suite_version").notNull(), label: text("label").notNull(), sourceReference: text("source_reference").notNull(), engineAlias: text("engine_alias").notNull(), modelVersion: text("model_version").notNull(), minimumConfidenceBps: integer("minimum_confidence_bps").notNull(), status: text("status").notNull().default("draft"), preparedByUserId: text("prepared_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), version: integer("version").notNull().default(1), ...timestamps,
}, (table) => [uniqueIndex("idx_report_reader_suites_version").on(table.suiteVersion), index("idx_report_reader_suites_status_updated").on(table.status, table.updatedAt)]);

export const reportReaderCases = sqliteTable("report_reader_cases", {
  id: text("id").primaryKey(), suiteId: text("suite_id").notNull().references(() => reportReaderSuites.id, { onDelete: "restrict" }), caseKey: text("case_key").notNull(), locale: text("locale").notNull(), reportType: text("report_type").notNull(), sourceReference: text("source_reference").notNull(), sourceChecksumSha256: text("source_checksum_sha256").notNull(), documentId: text("document_id").references(() => documentRecords.id, { onDelete: "restrict" }), documentVersion: integer("document_version"), extractedObservationsJson: text("extracted_observations_json").notNull(), expectedDecision: text("expected_decision").notNull(), status: text("status").notNull().default("review_required"), dataMode: text("data_mode").notNull().default("synthetic_only"), interpretationGenerated: integer("interpretation_generated", { mode: "boolean" }).notNull().default(false), reviewerProviderId: text("reviewer_provider_id").references(() => providerProfiles.id, { onDelete: "restrict" }), reviewDecision: text("review_decision"), reviewNote: text("review_note"), reviewedAt: integer("reviewed_at", { mode: "timestamp_ms" }), version: integer("version").notNull().default(1), ...timestamps,
}, (table) => [uniqueIndex("idx_report_reader_cases_suite_key").on(table.suiteId, table.caseKey), index("idx_report_reader_cases_status_created").on(table.status, table.createdAt), index("idx_report_reader_cases_reviewer_reviewed").on(table.reviewerProviderId, table.reviewedAt)]);

export const reportReaderEvents = sqliteTable("report_reader_events", {
  id: text("id").primaryKey(), caseId: text("case_id").notNull().references(() => reportReaderCases.id, { onDelete: "restrict" }), actorUserId: text("actor_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), action: text("action").notNull(), previousStatus: text("previous_status"), nextStatus: text("next_status").notNull(), note: text("note").notNull(), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_report_reader_events_case_created").on(table.caseId, table.createdAt)]);

export const reportReaderEvaluationRuns = sqliteTable("report_reader_evaluation_runs", {
...
export const medicationReminderSchedulerRuns = sqliteTable("medication_reminder_scheduler_runs", {
  id: text("id").primaryKey(), suiteId: text("suite_id").notNull().references(() => medicationReminderSchedulerSuites.id, { onDelete: "restrict" }), executedByUserId: text("executed_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), totalScenarios: integer("total_scenarios").notNull(), passedScenarios: integer("passed_scenarios").notNull(), failedScenarios: integer("failed_scenarios").notNull(), duplicateOccurrences: integer("duplicate_occurrences").notNull(), invalidSourceOccurrences: integer("invalid_source_occurrences").notNull(), deliveryAttempts: integer("delivery_attempts").notNull(), result: text("result").notNull(), failuresJson: text("failures_json").notNull().default("[]"), dataMode: text("data_mode").notNull().default("synthetic_only"), status: text("status").notNull().default("pending_review"), reviewerUserId: text("reviewer_user_id").references(() => users.id, { onDelete: "restrict" }), reviewedAt: integer("reviewed_at", { mode: "timestamp_ms" }), reviewNote: text("review_note"), version: integer("version").notNull().default(1), executedAt: integer("executed_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_med_reminder_scheduler_runs_suite_executed").on(table.suiteId, table.executedAt), index("idx_med_reminder_scheduler_runs_status_executed").on(table.status, table.executedAt), index("idx_med_reminder_scheduler_runs_reviewer_reviewed").on(table.reviewerUserId, table.reviewedAt)]);

export const medicationReminderSchedulerRunEvents = sqliteTable("medication_reminder_scheduler_run_events", {
  id: text("id").primaryKey(), runId: text("run_id").notNull().references(() => medicationReminderSchedulerRuns.id, { onDelete: "restrict" }), actorUserId: text("actor_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), action: text("action").notNull(), previousStatus: text("previous_status").notNull(), nextStatus: text("next_status").notNull(), note: text("note").notNull(), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_med_reminder_scheduler_run_events_run_created").on(table.runId, table.createdAt)]);

export const medicationReminderDeliveryPolicies = sqliteTable("medication_reminder_delivery_policies", {
  id: text("id").primaryKey(), policyVersion: text("policy_version").notNull(), label: text("label").notNull(), templateEn: text("template_en").notNull(), templateAr: text("template_ar").notNull(), consentVersion: text("consent_version").notNull(), timezone: text("timezone").notNull().default("Asia/Qatar"), quietHoursStart: text("quiet_hours_start").notNull(), quietHoursEnd: text("quiet_hours_end").notNull(), quietHoursBehavior: text("quiet_hours_behavior").notNull(), maximumLatenessMinutes: integer("maximum_lateness_minutes").notNull(), maxAttempts: integer("max_attempts").notNull(), retryBackoffMinutesJson: text("retry_backoff_minutes_json").notNull(), dedupeWindowMinutes: integer("dedupe_window_minutes").notNull(), primaryOwnerUserId: text("primary_owner_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), backupOwnerUserId: text("backup_owner_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), preparedByUserId: text("prepared_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), reviewerUserId: text("reviewer_user_id").references(() => users.id, { onDelete: "restrict" }), status: text("status").notNull().default("draft"), reviewedAt: integer("reviewed_at", { mode: "timestamp_ms" }), reviewNote: text("review_note"), version: integer("version").notNull().default(1), ...timestamps,
}, (table) => [uniqueIndex("idx_med_reminder_delivery_policies_version").on(table.policyVersion), index("idx_med_reminder_delivery_policies_status_updated").on(table.status, table.updatedAt), index("idx_med_reminder_delivery_policies_owner_status").on(table.primaryOwnerUserId, table.status)]);

export const medicationReminderDeliveryPolicyEvents = sqliteTable("medication_reminder_delivery_policy_events", {
  id: text("id").primaryKey(), policyId: text("policy_id").notNull().references(() => medicationReminderDeliveryPolicies.id, { onDelete: "restrict" }), actorUserId: text("actor_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), action: text("action").notNull(), previousStatus: text("previous_status"), nextStatus: text("next_status").notNull(), note: text("note").notNull(), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_med_reminder_delivery_policy_events_policy_created").on(table.policyId, table.createdAt)]);

export const medicationReminderDeliveryConsents = sqliteTable("medication_reminder_delivery_consents", {
  id: text("id").primaryKey(), userId: text("user_id").notNull().references(() => users.id, { onDelete: "restrict" }), consentVersion: text("consent_version").notNull(), locale: text("locale").notNull(), status: text("status").notNull().default("not_consented"), acknowledgedBoundariesJson: text("acknowledged_boundaries_json").notNull().default("[]"), consentedAt: integer("consented_at", { mode: "timestamp_ms" }), withdrawnAt: integer("withdrawn_at", { mode: "timestamp_ms" }), version: integer("version").notNull().default(1), ...timestamps,
}, (table) => [uniqueIndex("idx_med_reminder_delivery_consents_user").on(table.userId), index("idx_med_reminder_delivery_consents_status_updated").on(table.status, table.updatedAt)]);

export const medicationReminderDeliveryConsentEvents = sqliteTable("medication_reminder_delivery_consent_events", {
  id: text("id").primaryKey(), consentId: text("consent_id").notNull().references(() => medicationReminderDeliveryConsents.id, { onDelete: "restrict" }), actorUserId: text("actor_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), action: text("action").notNull(), previousStatus: text("previous_status"), nextStatus: text("next_status").notNull(), consentVersion: text("consent_version").notNull(), note: text("note").notNull(), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_med_reminder_delivery_consent_events_consent_created").on(table.consentId, table.createdAt)]);

export const medicationReminderDeliveryRehearsals = sqliteTable("medication_reminder_delivery_rehearsals", {
  id: text("id").primaryKey(), policyId: text("policy_id").notNull().references(() => medicationReminderDeliveryPolicies.id, { onDelete: "restrict" }), schedulerRunId: text("scheduler_run_id").notNull().references(() => medicationReminderSchedulerRuns.id, { onDelete: "restrict" }), suiteVersion: text("suite_version").notNull(), totalScenarios: integer("total_scenarios").notNull(), passedScenarios: integer("passed_scenarios").notNull(), failedScenarios: integer("failed_scenarios").notNull(), duplicateDeliveries: integer("duplicate_deliveries").notNull(), externalDeliveryAttempts: integer("external_delivery_attempts").notNull(), patientRecordsTouched: integer("patient_records_touched").notNull(), result: text("result").notNull(), failuresJson: text("failures_json").notNull().default("[]"), dataMode: text("data_mode").notNull().default("synthetic_only"), executedByUserId: text("executed_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), executedAt: integer("executed_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_med_reminder_delivery_rehearsals_result_executed").on(table.result, table.executedAt), index("idx_med_reminder_delivery_rehearsals_policy_executed").on(table.policyId, table.executedAt)]);
...
export const guardianNominations = sqliteTable("guardian_nominations", {
  id: text("id").primaryKey(), dependentId: text("dependent_id").notNull().references(() => dependentProfiles.id, { onDelete: "restrict" }), nominatedByUserId: text("nominated_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), nomineeEmail: text("nominee_email").notNull(), tokenHash: text("token_hash").notNull(), authorityType: text("authority_type").notNull(), status: text("status").notNull().default("pending"), acceptedByUserId: text("accepted_by_user_id").references(() => users.id, { onDelete: "restrict" }), expiresAt: integer("expires_at", { mode: "timestamp_ms" }).notNull(), acceptedAt: integer("accepted_at", { mode: "timestamp_ms" }), ...timestamps,
}, (table) => [uniqueIndex("idx_guardian_nominations_token").on(table.tokenHash), index("idx_guardian_nominations_dependent_status").on(table.dependentId, table.status), index("idx_guardian_nominations_email_status").on(table.nomineeEmail, table.status)]);

export const dependentTransitionRehearsals = sqliteTable("dependent_transition_rehearsals", {
  id: text("id").primaryKey(), dependentId: text("dependent_id").notNull().references(() => dependentProfiles.id, { onDelete: "restrict" }), rehearsalVersion: text("rehearsal_version").notNull(), scenarioCount: integer("scenario_count").notNull(), passedScenarios: integer("passed_scenarios").notNull(), failedScenarios: integer("failed_scenarios").notNull(), adultAccountsCreated: integer("adult_accounts_created").notNull(), authoritiesActivated: integer("authorities_activated").notNull(), emergencyAccessGrants: integer("emergency_access_grants").notNull(), result: text("result").notNull(), dataMode: text("data_mode").notNull().default("synthetic_only"), executedByUserId: text("executed_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), executedAt: integer("executed_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_dependent_transition_rehearsals_dependent_executed").on(table.dependentId, table.executedAt), index("idx_dependent_transition_rehearsals_result_executed").on(table.result, table.executedAt)]);
...
export const appointmentWaitlistRequests = sqliteTable("appointment_waitlist_requests", { id:text("id").primaryKey(), patientId:text("patient_id").notNull().references(()=>patientProfiles.id,{onDelete:"restrict"}), providerId:text("provider_id").notNull().references(()=>providerProfiles.id,{onDelete:"restrict"}), serviceLocationId:text("service_location_id").notNull().references(()=>providerServiceLocations.id,{onDelete:"restrict"}), facilityId:text("facility_id").references(()=>facilities.id,{onDelete:"restrict"}), mode:text("mode").notNull(), earliestDate:text("earliest_date").notNull(), latestDate:text("latest_date").notNull(), timePreference:text("time_preference").notNull(), status:text("status").notNull().default("waiting"), version:integer("version").notNull().default(1), ...timestamps },table=>[index("idx_waitlist_patient_status_updated").on(table.patientId,table.status,table.updatedAt),index("idx_waitlist_provider_status_created").on(table.providerId,table.status,table.createdAt)]);
export const appointmentWaitlistOffers = sqliteTable("appointment_waitlist_offers", { id:text("id").primaryKey(), requestId:text("request_id").notNull().references(()=>appointmentWaitlistRequests.id,{onDelete:"restrict"}), scheduledStart:integer("scheduled_start",{mode:"timestamp_ms"}).notNull(), scheduledEnd:integer("scheduled_end",{mode:"timestamp_ms"}).notNull(), expiresAt:integer("expires_at",{mode:"timestamp_ms"}).notNull(), status:text("status").notNull().default("offered"), appointmentId:text("appointment_id").references(()=>appointments.id,{onDelete:"restrict"}), version:integer("version").notNull().default(1), ...timestamps },table=>[index("idx_waitlist_offers_request_status_created").on(table.requestId,table.status,table.createdAt),index("idx_waitlist_offers_status_expires").on(table.status,table.expiresAt)]);
export const appointmentWaitlistEvents = sqliteTable("appointment_waitlist_events", { id:text("id").primaryKey(), requestId:text("request_id").notNull().references(()=>appointmentWaitlistRequests.id,{onDelete:"restrict"}), actorUserId:text("actor_user_id").notNull().references(()=>users.id,{onDelete:"restrict"}), action:text("action").notNull(), previousStatus:text("previous_status"), nextStatus:text("next_status").notNull(), createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull() },table=>[index("idx_waitlist_events_request_created").on(table.requestId,table.createdAt)]);
export const appointmentWaitlistRehearsals = sqliteTable("appointment_waitlist_rehearsals", { id:text("id").primaryKey(), rehearsalVersion:text("rehearsal_version").notNull(), scenarioCount:integer("scenario_count").notNull(), passedScenarios:integer("passed_scenarios").notNull(), failedScenarios:integer("failed_scenarios").notNull(), requestsCreated:integer("requests_created").notNull(), offersCreated:integer("offers_created").notNull(), appointmentsCreated:integer("appointments_created").notNull(), externalMessagesSent:integer("external_messages_sent").notNull(), result:text("result").notNull(), dataMode:text("data_mode").notNull(), executedByUserId:text("executed_by_user_id").notNull().references(()=>users.id,{onDelete:"restrict"}), executedAt:integer("executed_at",{mode:"timestamp_ms"}).notNull() },table=>[index("idx_waitlist_rehearsals_result_executed").on(table.result,table.executedAt)]);
